...
interface DataObject {
  filters: {
    ...
    location: string;
    topic: string;
    job?: string; // Optional if job is not always present
  };
  summaries: Array<{
    title: string;
    description: string;
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="relative col-span-2 flex flex-col items-start justify-center rounded-[10px] bg-white p-10 shadow-1 dark:bg-gray-dark">
          ...
          <ul className="mt-4 list-disc pl-5 text-sm text-gray-700 dark:text-gray-300">
            <li>Age: {data.filters.age}</li>
            <li>Sesso: {data.filters.gender}</li>
            {data.filters.job && <li>Occupazione: {data.filters.job}</li>}
          </ul>
        </div>
